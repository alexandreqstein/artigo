...
files <- list.files(path = "1_dados", pattern = "cnae")
...
foreach(k = seq_along(files)) %dopar% {
    ...
    complexity <- readRDS(paste0("3_results_4d/PCI_", anos[k], ".RDS"))
    ...
    as.matrix(proximities[[2]]) %>%
        ...
        rename(source = Var1,
               target = Var2,
               weight = Freq) %>% 
        saveRDS(str_c("3_results_4d_cnae/similarities_", anos[k], ".RDS"))
    ...
    write.csv(edgelist_product, str_c("3_results_4d_cnae/OS_edgelist_", anos[k], ".csv"), row.names = FALSE)
    write.csv(nodes_list_product, str_c("3_results_4d_cnae/OS_nodelist_", anos[k], ".csv"), row.names = FALSE)
