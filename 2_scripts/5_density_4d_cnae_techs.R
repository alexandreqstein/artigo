...
files_densidade <- list.files(path = "3_results_4d_cnae", pattern = "similarities", full.names = TRUE)
...
dados_completos <- foreach( k = seq_along(files_densidade)) %dopar% {
    ...
    data_mat <- readRDS(paste0("1_dados/", files_muns[k])) %>%
        mutate(cbo_2002 = str_sub(cbo_2002, 1, 4)) %>% 
        filter(cbo_2002 %in% cbos_interesse) %>% 
    ...
    proximidades <- read_rds(files_densidade[k]) %>%
        filter(source %in% cbos_interesse & target %in% cbos_interesse) %>% 
...
fwrite(dados_completos, "3_results_4d_cnae/densidade_techs.csv",
       scipen = getOption("scipen", 999L))



saveRDS(dados_completos, "3_results_4d_cnae/densidade_techs.RDS")
