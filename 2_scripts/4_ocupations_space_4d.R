...
foreach(k = seq_along(files)) %dopar% {
    ...
    as.matrix(proximities[[2]]) %>%
        ...
        rename(source = Var1,
               target = Var2,
               weight = Freq) %>% 
        saveRDS(str_c("3_results_4d/similarities_", anos[k], ".RDS"))
    ...
    write.csv(edgelist_product, str_c("3_results_4d/OS_edgelist_", anos[k], ".csv"), row.names = FALSE)
    write.csv(nodes_list_product, str_c("3_results_4d/OS_nodelist_", anos[k], ".csv"), row.names = FALSE)
